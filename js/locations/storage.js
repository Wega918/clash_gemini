(function(){
  var MODAL_ID = 'storage-modal';
  var CONTENT_ID = 'storage-modal-content';
  var ENDPOINT = 'ajax.php?page=storage';
  var TITLE = 'ХРАНИЛИЩА';
...
function applyBalancePayload(root){
  ...
  function ensureEls(){
    ...
    if (!modal || !cont) {
      console.error('Storage modal container not found');
      return null;
    }
  ...
  // Public API
  window.showStorageModal = function(view){
    historyStack = [];
    loadView(view || 'main', '', 0, false);
  };

  window.storageLoadList = function(modalId, buildingType){
    // modalId параметр оставлен для совместимости с шаблонами
    loadView('list', buildingType, 0, true);
  };

  window.storageLoadDetail = function(modalId, buildingRowId){
    loadView('detail', '', buildingRowId, true);
  };

  window.storageGoBack = function(modalId, defaultView){
    // возвращаемся к предыдущему состоянию, иначе в defaultView
  ...
  window.storageCollectResource = function(modalId, buildingRowId){
    loadView('collect', '', buildingRowId, false);
  };

  window.storageStartUpgrade = function(modalId, buildingRowId){
    loadView('upgrade', '', buildingRowId, false);
  };

  window.storageStartBuilding = function(modalId, buildingType){
